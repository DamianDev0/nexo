...
```
         ...
         /──────\
        /Integration\    25% — Jest + Supertest + BD real (Docker)
       /──────────────\
      /   Unit Tests   \  70% — Jest — services, validators, utilities
     /──────────────────\
```

**Framework principal:** Jest (más rápido que Jest, nativo con ESM)  
**E2E:** Playwright  
...
test('Flujo completo: registro → contacto → deal → factura DIAN → link de pago', async ({
  page,
}) => {
  // 1. Registro
...
_Test Strategy generado por QA Quinn (TEA Module) — BMad Enterprise Track | NexoCRM v1.0_
