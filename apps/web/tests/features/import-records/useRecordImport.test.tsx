...
import { useRecordImport } from '@/features/import-records/model/useRecordImport'
...
async function analyzed(onFinished = vi.fn()) {
  const view = renderHook(() => useRecordImport(onFinished), { wrapper })
...
describe('useRecordImport', () => {
  it('starts on the upload step with nothing loaded', () => {
    const { result } = renderHook(() => useRecordImport(vi.fn()), { wrapper })
