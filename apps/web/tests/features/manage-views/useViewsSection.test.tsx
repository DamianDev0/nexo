...
import { useViewsSection } from '@/features/manage-views'
...
describe('useViewsSection', () => {
  it('exposes saved views as smart list items', () => {
    const { result } = renderHook(() => useViewsSection([VIEW], SNAPSHOT, handlers(), VIEW.ownerId))
  ...
  it('marks views as shared when they belong to someone else or the viewer is unknown', () => {
    const other = renderHook(() => useViewsSection([VIEW], SNAPSHOT, handlers(), 'u2'))
    const unknown = renderHook(() => useViewsSection([VIEW], SNAPSHOT, handlers(), null))
  ...
  it('applies the stored view state when its list item is selected', () => {
    ...
    const { result } = renderHook(() => useViewsSection([VIEW], SNAPSHOT, apply))
