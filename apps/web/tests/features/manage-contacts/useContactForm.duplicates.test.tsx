...
describe('useContactForm duplicates', () => {
  it('shows an inline email error on a hard duplicate and keeps the drawer open', async () => {
    server.use(
      ...
      http.post(`${API}/contacts`, () =>
        conflictResponse({ severity: 'hard', field: 'email', matches: [match('email')] }),
      ),
    )
    const onDone = vi.fn()
  ...
  it('offers force on a soft phone duplicate and retries the same mutation with force', async () => {
    ...
    server.use(
      ...
      http.post(`${API}/contacts`, async ({ request }) => {
        ...
        if (force !== 'true') {
          return conflictResponse({ severity: 'soft', field: 'phone', matches: [match('phone')] })
        }
  ...
  it('clears the duplicate notice when the user edits a value', async () => {
    server.use(
      taxonomyHandler(),
      http.post(`${API}/contacts`, () =>
        conflictResponse({ severity: 'soft', field: 'phone', matches: [match('phone')] }),
      ),
    )
