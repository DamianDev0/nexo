...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({ i18n: { resolvedLanguage } }),
}))
...
describe('useSwitchLanguage', () => {
  ...
  it('switches locale: persists the cookie, then reloads for a coherent server tree', async () => {
    const { result } = renderHook(() => useSwitchLanguage())
  ...
  it('reloads only after the cookie is persisted', async () => {
    const order: string[] = []
    setLocaleAction.mockImplementationOnce(async () => {
      order.push('cookie')
    })
    reload.mockImplementationOnce(() => order.push('reload'))
    const { result } = renderHook(() => useSwitchLanguage())
    ...
    expect(order).toEqual(['cookie', 'reload'])
  })
