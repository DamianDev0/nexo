...
function renderPatchHook(mutationFn: (change: NameChange) => Promise<unknown>) {
  ...
  const { result } = renderHook(
    () =>
      useOptimisticRecordListPatch<ContactListItem, NameChange>({
        mutationFn,
...
describe('useOptimisticRecordListPatch', () => {
...
describe('useOptimisticRecordListPatch — overlapping mutations', () => {
  it('only reverts its own fields when an earlier mutation fails after a later one applied', async () => {
    ...
    const second = renderHook(
      () =>
        useOptimisticRecordListPatch<ContactListItem, { id: string; lastName: string }>({
  ...
  it('flags the patched contact as pending until the request settles', async () => {
    ...
    await waitFor(() => expect(usePendingRecordPatches.getState().ids.has('c1')).toBe(true))

    release()
    await waitFor(() => expect(usePendingRecordPatches.getState().ids.has('c1')).toBe(false))
  })
})
