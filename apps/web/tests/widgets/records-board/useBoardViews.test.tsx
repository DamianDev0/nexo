...
import { EMPTY_TABLE_STATE } from '@/widgets/records-board/config/board-empty.constants'
import { useBoardViews, type BoardViewsTable } from '@/widgets/records-board/model/useBoardViews'
...
function buildView(overrides: Partial<ObjectView> = {}): ObjectView {
  return {
    id: 'v1',
    ownerId: 'viewer-1',
    name: 'My view',
...
function makeTable(overrides: Partial<BoardViewsTable> = {}): BoardViewsTable {
  return {
    advanced: [],
    search: '',
    sort: null,
...
describe('useBoardViews', () => {
  ...
  it('updates the snapshot sort once the table sort changes', () => {
    ...
    const { result, rerender } = renderHook(
      ({ sort }: { sort: BoardViewsTable['sort'] }) =>
        useBoardViews({
