...
import { useSkeletonHintSync } from '@/widgets/records-board/model/useSkeletonHintSync'
...
vi.mock('@/entities/object-descriptor', async (importOriginal) => ({
  ...
  useObjectDescriptor: () => ({ queryRoot: 'contacts' }),
  writeSkeletonHint: (queryRoot: string, hint: unknown) => writeSkeletonHint(queryRoot, hint),
}))
...
describe('useSkeletonHintSync', () => {
  ...
  it('falls back to five rows when the row count is not yet known', () => {
    ...
    expect(writeSkeletonHint).toHaveBeenCalledWith('contacts', { widths: [40, 50, 60], rows: 5 })
  })
  ...
  it('stores the exact row count once rows are loaded', () => {
    ...
    expect(writeSkeletonHint).toHaveBeenCalledWith('contacts', { widths: [40, 50, 60], rows: 12 })
  })
  ...
  it('writes again once the hint content actually changes', () => {
    ...
    expect(writeSkeletonHint).toHaveBeenLastCalledWith('contacts', {
      widths: [40, 50, 60],
      rows: 20,
    })
  })
