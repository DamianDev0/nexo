    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <PillButton
          ...
          size="xs"
          aria-label={label}
          className="size-7 shrink-0 rounded-md border border-border px-0 text-muted-foreground hover:text-foreground"
        >
          ...
          {ACTIVITY_ICON_NAMES.map((name) => {
            ...
            return (
              <PillButton
                ...
                className={cn('w-8 px-0', name === icon && 'bg-muted text-primary')}
                onClick={() => {
                  onChange(name)
                  setOpen(false)
                }}
