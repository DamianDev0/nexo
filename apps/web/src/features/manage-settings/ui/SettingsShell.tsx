...
export function SettingsShell({ children }: Readonly<{ children: ReactNode }>) {
  ...
  return (
    <div className="flex h-full">
      <aside className="scrollbar-hidden w-56 shrink-0 overflow-y-auto border-r border-border px-3 py-6">
        <SettingsNav />
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div ref={scrollRef} className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto px-8 py-6">
          <motion.div
            key={pathname}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
