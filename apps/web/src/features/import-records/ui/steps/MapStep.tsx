...
export function MapStep({ analysis, data, onRemap }: Readonly<MapStepProps>) {
  ...
  const { config } = useImportDescriptor()
    ...
    <div className="flex flex-col gap-6">
          ...
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="h-9 text-xs font-medium">
                {t('imports.map.columnHeader')}
              </TableHead>
              <TableHead className="h-9 text-xs font-medium">
                {t('imports.map.sampleHeader')}
              </TableHead>
              <TableHead className="h-9 w-56 text-xs font-medium">
                {t('imports.map.fieldHeader')}
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              <TableRow key={column.csvColumn} className="hover:bg-transparent">
                ...
                <TableCell className="max-w-64">
                  <TruncateTip className="text-muted-foreground">
                    {column.sampleValues.slice(0, 3).join(' · ') || t('imports.map.empty')}
                  </TruncateTip>
                </TableCell>
                <TableCell>
                  <FieldSelector
      ...
      {data.missingFields.length > 0 && (
        <Text as="p" role="alert" className="text-destructive">
          {t('imports.map.missing', {
            fields: data.missingFields
              .map((field) => fieldLabel(t, config.fieldLabelKey, field))
              .join(', '),
          })}
        </Text>
      )}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-baseline justify-between gap-2">
          <Text variant="strong">{t('imports.preview.title')}</Text>
          <Text variant="hint">
            {t('imports.preview.counts', { valid: counts.valid, invalid: counts.invalid })}
          </Text>
        </div>
        <ImportPreview
          preview={preview}
