...
import { IMPORT_UNMAPPED } from '../config/import.constants'
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="truncate">
            {selected ? fieldLabel(t, config.fieldLabelKey, selected) : t('imports.map.ignore')}
          </span>
      ...
      <PopoverContent align="end" className="w-64 p-0">
        <Command>
          <CommandInput placeholder={t('imports.map.search')} />
          <CommandList>
            <CommandEmpty>{t('common.noResults')}</CommandEmpty>
            <CommandGroup>
              <CommandItem
                value={t('imports.map.ignore')}
                onSelect={() => {
              ...
              {fields.map((field) => (
                <CommandItem
                  key={field.field}
                  value={`${fieldLabel(t, config.fieldLabelKey, field)} ${field.field}`}
                  onSelect={() => {
                ...
                >
                  <span className="flex-1">
                    {fieldLabel(t, config.fieldLabelKey, field)}
