...
const segClass = (active: boolean, hasValue: boolean) =>
  cn(
    ...
    hasValue ? 'text-neutral-900 dark:text-neutral-100' : 'text-neutral-400 dark:text-neutral-600',
  )
...
function CalendarScheduler({
  ...
  const isToday = useCallback(
    (d: number) => d === now.getDate() && month === now.getMonth() && year === now.getFullYear(),
    [now, month, year],
  )
        ...
        {open === 'day' && (
          ...
          >
            ...
            >
              ...
              >
                {grid.map((d, i) => {
                  if (d === null) return <div key={`e${i}`} style={{ padding: 8 }} />
