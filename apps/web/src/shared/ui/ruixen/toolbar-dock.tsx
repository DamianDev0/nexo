...
function offsetLeftWithin(el: HTMLElement | null, ancestor: HTMLElement | null): number {
  let x = 0
  ...
  const renderButton = (item: ToolbarDockItem, index: number) => {
    ...
    return (
      <button
        ...
        aria-label={isToggle ? (collapsed ? 'Expand toolbar' : 'Collapse toolbar') : undefined}
      ...
      >
        <div className="group relative flex size-8 items-center justify-center rounded-full p-1.5 transition-colors hover:bg-accent">
          <span className="h-full w-full [&>svg]:h-full [&>svg]:w-full">{item.icon}</span>
          {item.badge && (
