...
export function DrumPicker({
  ...
  const defaultIdx = defaultValue ? Math.max(0, items.indexOf(defaultValue)) : 0
  const [internalIdx, setInternalIdx] = useState(defaultIdx)

  const isControlled = controlledValue !== undefined
  const currentIdx = isControlled ? Math.max(0, items.indexOf(controlledValue)) : internalIdx
  ...
  const onMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const nearIdx = clamp(Math.round(newAngle / ANGLE_STEP), 0, items.length - 1)
  ...
  const onUp = useCallback(() => {
    if (!isDragging) return
    setIsDragging(false)
    const nearIdx = clamp(Math.round(dragAngle / ANGLE_STEP), 0, items.length - 1)
    set(nearIdx)
