import { type INestApplication } from '@nestjs/common'
import request from 'supertest'

import { createTestApp, onboardTenant, asTenant, teardownTenants, API_PREFIX } from './helpers/e2e'
import type { TestApp, OnboardedTenant } from './helpers/e2e'

describe('Products Tenant Isolation (E2E, real HTTP)', () => {
  let ctx: TestApp
  let app: INestApplication
  let tenantA: OnboardedTenant
  let tenantB: OnboardedTenant

  const SLUG_A = 'iso-products-a'
  const SLUG_B = 'iso-products-b'

  beforeAll(async () => {
    ctx = await createTestApp()
    app = ctx.app
    await teardownTenants(ctx, [SLUG_A, SLUG_B])
    tenantA = await onboardTenant(app, SLUG_A)
    tenantB = await onboardTenant(app, SLUG_B)
  })

  afterAll(async () => {
    await teardownTenants(ctx, [SLUG_A, SLUG_B])
    await app.close()
  })

  it('provisions two tenants with isolated schemas', () => {
    expect(tenantA.schemaName).toBe('tenant_iso_products_a')
    expect(tenantB.schemaName).toBe('tenant_iso_products_b')
    expect(tenantA.tenantId).not.toBe(tenantB.tenantId)
  })

  it("does NOT expose tenant A's product to tenant B (404)", async () => {
    const created = await asTenant(
      request(app.getHttpServer()).post(`/${API_PREFIX}/products`),
      tenantA,
    )
      .send({ name: 'Widget Pro', sku: 'WGT-001', priceCents: 149900 })
      .expect(201)

    const productId = created.body.data.id as string
    expect(productId).toBeTruthy()

    await asTenant(
      request(app.getHttpServer()).get(`/${API_PREFIX}/products/${productId}`),
      tenantA,
    ).expect(200)

    await asTenant(
      request(app.getHttpServer()).get(`/${API_PREFIX}/products/${productId}`),
      tenantB,
    ).expect(404)
  })

  it('rejects unauthenticated access to the products list', async () => {
    await request(app.getHttpServer())
      .get(`/${API_PREFIX}/products`)
      .set('x-tenant-slug', SLUG_A)
      .expect(401)
  })
})
