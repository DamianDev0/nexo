import { type INestApplication } from '@nestjs/common'
import request from 'supertest'

import { createTestApp, onboardTenant, asTenant, teardownTenants, API_PREFIX } from './helpers/e2e'
import type { TestApp, OnboardedTenant } from './helpers/e2e'

describe('Activities Tenant Isolation (E2E, real HTTP)', () => {
  let ctx: TestApp
  let app: INestApplication
  let tenantA: OnboardedTenant
  let tenantB: OnboardedTenant

  const SLUG_A = 'iso-activities-a'
  const SLUG_B = 'iso-activities-b'

  beforeAll(async () => {
    ctx = await createTestApp()
    app = ctx.app
    await teardownTenants(ctx, [SLUG_A, SLUG_B])
    tenantA = await onboardTenant(app, SLUG_A)
    tenantB = await onboardTenant(app, SLUG_B)
  })

  afterAll(async () => {
    await teardownTenants(ctx, [SLUG_A, SLUG_B])
    await app.close()
  })

  it('provisions two tenants with isolated schemas', () => {
    expect(tenantA.schemaName).toBe('tenant_iso_activities_a')
    expect(tenantB.schemaName).toBe('tenant_iso_activities_b')
    expect(tenantA.tenantId).not.toBe(tenantB.tenantId)
  })

  it("does NOT expose tenant A's activity to tenant B (404)", async () => {
    const created = await asTenant(
      request(app.getHttpServer()).post(`/${API_PREFIX}/activities`),
      tenantA,
    )
      .send({ activityType: 'call', title: 'Follow-up con cliente A' })
      .expect(201)

    const activityId = created.body.data.id as string
    expect(activityId).toBeTruthy()

    await asTenant(
      request(app.getHttpServer()).get(`/${API_PREFIX}/activities/${activityId}`),
      tenantA,
    ).expect(200)

    await asTenant(
      request(app.getHttpServer()).get(`/${API_PREFIX}/activities/${activityId}`),
      tenantB,
    ).expect(404)
  })

  it('rejects unauthenticated access to the activities list (401)', async () => {
    await request(app.getHttpServer())
      .get(`/${API_PREFIX}/activities`)
      .set('x-tenant-slug', SLUG_A)
      .expect(401)
  })
})
