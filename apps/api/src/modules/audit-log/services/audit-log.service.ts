...
import { AuditAction, AuditEntityType } from '../interfaces/audit-log.interfaces'
import type { AuditEvent, AuditMeta, UserRef, TenantRef } from '../interfaces/audit-log.interfaces'
import type {
  AuditLogFilters,
  AuditLogPage,
  AuditLogRow,
...
export class AuditLogService {
  ...
  async findPage(schemaName: string, filters: AuditLogFilters): Promise<AuditLogPage> {
    return this.repo.findPage(schemaName, filters)
  }
  ...
  async findAll(
    ...
    return this.repo.findAll(schemaName, filters)
  }
  ...
  private async write(event: AuditEvent): Promise<void> {
    try {
      ...
      this.logger.error(
        { error: message, action: event.action, schemaName: event.schemaName },
