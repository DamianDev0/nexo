...
export class TenantProvisioningService {
  ...
  async createTenantSchema(schemaName: string): Promise<void> {
    ...
    try {
      ...
      // Enable pgvector in the schema (requires extension on the database)
      await queryRunner.query(`CREATE EXTENSION IF NOT EXISTS vector`)

      // Create all tenant tables
      await queryRunner.query(this.getTenantSchemaSQL(schemaName))

      // Create indices
      await queryRunner.query(this.getTenantIndicesSQL(schemaName))

      await queryRunner.commitTransaction()
      this.logger.log(`Tenant schema created: ${schemaName}`)
    } catch (error) {
      await queryRunner.rollbackTransaction()
      this.logger.error(`Failed to create tenant schema: ${schemaName}`, (error as Error).stack)
      throw error
    } finally {
   ...
   * Intended for testing and cleanup only.
   */
  async dropTenantSchema(schemaName: string): Promise<void> {
    ...
    this.logger.warn(`Tenant schema dropped: ${schemaName}`)
  }

  private getTenantSchemaSQL(schema: string): string {
    return `
  ...
  private getTenantIndicesSQL(schema: string): string {
    return `
